    ...
    for (const chatId of TELEGRAM_CHAT_IDS) {
      try {
        ...
        if (filePath && fs.existsSync(filePath)) {
          ...
          console.log(`✅ Документ надіслано в Telegram (chat_id: ${chatId}):`, result);
        } else {
          ...
          const result = await tgRes.text();
          console.log(`⚠️ Без файлу. Повідомлення надіслано (chat_id: ${chatId}):`, result);
        }
      } catch (err) {
        console.error(`❌ Telegram error (chat_id: ${chatId}):`, err.message);
      }
    }
  }
